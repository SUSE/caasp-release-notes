...
{productname} did not detect existed metrics-server installed by the administrator, so please remove the installed metrics-server manually before upgrade addons.
