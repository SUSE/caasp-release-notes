...
From {productname} 4.5.0 version Kubernetes packages are built with new package names for example:
...
Kubernetes packages use kubernetes-1.18 pattern name, because link:https://kubic.opensuse.org/[openSUSE Kubic] project already uses kubernetes1.18 but with different package (dependency) semantics. Having two identically named packages from SUSE (one CaaSP, one Kubic) with different semantics could lead to confusion and problems with installation. 
