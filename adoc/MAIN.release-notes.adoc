...
https://github.com/SUSE/avant-garde/milestone/7

Release criteria for Beta3:

Upgrade strategy

How do we roll out upgrades of the cluster ?

    upgrade of caasp container images
    upgrade of caasp node packages
    upgrade of caasp node kernels (-> reboot, kured ?!)
    upgrade of k8s to next major version
    upgrade of caasp node OS to next service pack (15 SP1 to 15 SP2)
...
The `caaspctl` tool will be renamed from it's working title to `skuba` with the release of {productname} Beta 3.
