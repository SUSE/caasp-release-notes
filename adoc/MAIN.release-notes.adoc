...
In order to update skuba, run the following command:

`zypper install skuba`

===
4.1.0 is considered a feature upgrade, and because of this, running `zypper patch` will not install 4.1.0. This is why you are requested to run `zypper install skuba`, instead.
===
...
[WARNING]
====
Please, do not run `zypper patch` manually on your nodes. If you do, you will see an error about a conflict when patching cri-o. This is expected, because the patch is not supposed to be installed this way. Instead, cluster updates are being handled by skuba link:https://documentation.suse.com/suse-caasp/4.1/single-html/caasp-admin/#handling_updates[documented in the Administration guide] 
====
