...
In this latest version Custom Resources Definitions (CRD) are out of the beta version and are now generally available in the `apiextensions.k8s.io/v1` group.
Additionally, the IPv4/IPv6 dual stack is now officially in alpha as of {kube} {kube_version}.
You can read about the details of the new features of {kube} {kube_version} here:
