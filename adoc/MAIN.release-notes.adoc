...
skuba should disable firewalld when bootstrapping/joining a node. Adds a startup step to check that firewalld is disabled. This was done using cloud init but this would not work on bare metal, so we make skuba ensure firewalld is disabled.
...
==== skuba-update error: patterns-caasp-Node conflicting with cri-o update

If your cluster is not up-to-date, meaning it is not in the latest kubernetes version, skuba-update will try to install the latest cri-o, which will create a conflict with
the current installed kubernetes packages.

More precisly, you will see an error similar to this one:

"patterns-caasp-Node conflicts with cri-o"

The solution is to upgrade the cluster by running `skuba node upgrade` for each node.
...
If it happens that your cluster is not in the latest kubernetes version prior to applying the update, you will encounter an issue when `skuba-update` tries to update your nodes. See the "Known bugs" section for how to proceed.
