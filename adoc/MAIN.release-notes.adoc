...
Finally, to apply the Prometheus `pushgateway` fix, enable it in your helm chart link:https://github.com/SUSE/kubernetes-charts-suse-com/blob/master/stable/prometheus/values.yaml#L848[] and use the `helm ugrade` command link:https://helm.sh/docs/intro/using_helm/#helm-upgrade-and-helm-rollback-upgrading-a-release-and-recovering-on-failure[].
