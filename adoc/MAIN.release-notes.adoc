...
You can now install `etcdctl`, the CLI to provide access to etcd cluster from the management host.

==== etcd Backup and Restore for Master Nodes Disaster Recovery

* Provide etcd backup process to protect etcd data corruption on-demand or by schedule.
...
You can now install `velero`, the CLI to provide access to backup cluster data to S3 storage or S3-compatible storage and recover/migrate to a different cluster.

==== Velero for Disaster Recovery

* Provide Velero as a solution for data protection and data migration by backing up and migrating Kubernetes resources and persistent volumes to and from externally supported storage backend on demand or by schedule. 
