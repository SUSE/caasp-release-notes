...
. Check upgrades at the node level
  ``skuba node upgrade plan <nodename>``
. Upgrade all control plane node, one by one, by running
  ``skuba node upgrade apply`` on each of them.
  This has to happen serially, but the order of the control nodes does not mater.
. Upgrade all worker nodes, one by one, by running ``skuba node upgrade apply``
  on each of then.

Then apply upgrade for addons in the cluster:

. Check the addon upgrade plan
