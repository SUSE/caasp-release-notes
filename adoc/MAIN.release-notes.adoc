...
* Dex and Gangway fixes (see below)
* skuba fixes (see below)
* supportutils-plugin-suse-caasp fixes (see below)

=== Required Actions

==== Dex/Gangway Adjust Replica Instructions

`oidc-dex/oidc-gangway` containers deployment configuration have been updated (see Bugs fixed below).
In order to adjust `oidc-dex/oidc-gangway` replica, see link:{docurl}/caasp-deployment/single-html/#_replicas[Replicas at the {productname} deployment guide]

=== Bugs Fixed in 4.1.1 since 4.1.0

* link:https://bugzilla.suse.com/show_bug.cgi?id=1157337[bsc#1157337] [dex/gangway] - Fixed after cluster creation all DEX and all GANGWAY pods run on the first master
* link:https://bugzilla.suse.com/show_bug.cgi?id=1157338[bsc#1157338] [skuba] - Fixed after cluster creation all DEX and all GANGWAY pods run on the first master
